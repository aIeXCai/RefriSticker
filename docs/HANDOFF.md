...
| 图像 | 火山引擎 Ark Doubao Seedream 5.0 lite、Canvas | 多图融合（用户照片 + 风格参考）生成底图，浏览器合成并下载成品（`app/vite.config.mjs:24`、`app/src/App.jsx:556`） |
...
- 火山引擎 Ark 经 `/api/generate-image` 服务端代理调用，避免 API Key 暴露给浏览器（`app/vite.config.mjs`）。
- 采用多图融合：图 1 是用户照片（提供主体识别度），图 2 是 `app/public/style-refs/` 下的风格参考图（提供艺术语言），由中间件在请求时本地注入（`app/vite.config.mjs:31`）。
...
## 7. 陷阱清单
- 不要把 ARK_API_KEY 放进前端代码，因为浏览器请求会暴露密钥（`app/vite.config.mjs`）。
...
- 不要把当前目录当作已有 Git 仓库，因为扫描时未发现 Git 元数据（仓库在 2026-06-22 初始化并推送到 GitHub）。
